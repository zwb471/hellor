    ...
    <a-list :data-source="subscribers" bordered >
      <template #renderItem="{ item }">
        <a-list-item>
          <a-list-item-meta
            :description="`
              ${codeToProvince(item['province'])} --
              ${moment(item['subscribeTime']).format('YYYY年MM月DD日 HH:mm:ss')}
            `"
          >
            <template #title>
            ...
            <template #avatar>
              <a-avatar :src="item['avatarUrl']+'?param=32y32'" />
            </template>
          </a-list-item-meta>
        </a-list-item>
      </template>
...
import {codeToProvince} from '@/utils/format'
